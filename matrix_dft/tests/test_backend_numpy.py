...
def test_MFT_flux_conservation(centering=constants.FFTSTYLE, precision=0.01):
    ...
    a = backend_numpy.matrix_dft(pupil, u, npix, centering=centering)
...
def test_DFT_rect(centering=constants.FFTSTYLE, npix=None, sampling=10.,
    ...
    a = backend_numpy.matrix_dft(pupil, nlamd, npix, centering=centering)
    ...
    # Inverse transform:
    pupil2 = backend_numpy.matrix_idft(a, u, npupil, centering=centering)
...
def test_check_invalid_centering():
    ...
    with pytest.raises(ValueError) as excinfo:
        mft = backend_numpy.matrix_dft(plane, 10, 10, centering='some garbage value')
...
def test_parity_MFT_forward_inverse(npix=512, centering=constants.ADJUSTABLE):
    ...
    **  parallel function to this.                                  **
    ...
    nlamD = npix
    mftout = backend_numpy.matrix_dft(pupil, nlamD, npix, centering=centering)
    imftout = backend_numpy.matrix_idft(mftout, nlamD, npix, centering=centering)
...
def test_MFT_FFT_equivalence():
    ...
    mftout = backend_numpy.matrix_dft(imgin, nlamD, npix, centering=centering)
